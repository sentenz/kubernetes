...
k8s-deploy-%:
	$(KUSTOMIZE_BIN) build manifests/overlays/$* --enable-helm --load-restrictor=LoadRestrictionsNone | kubectl apply --kubeconfig examples/config/kubeconfig.yaml -f -
.PHONY: k8s-deploy-%
...
k8s-destroy-%:
	$(KUSTOMIZE_BIN) build manifests/overlays/$* --enable-helm --load-restrictor=LoadRestrictionsNone | kubectl delete --kubeconfig examples/config/kubeconfig.yaml -f -
.PHONY: k8s-destroy-%

## List all services
k8s-list-service:
	kubectl get services --kubeconfig examples/config/kubeconfig.yaml
.PHONY: k8s-list-service

## List all namespaces
k8s-list-namespace:
	kubectl get namespaces --kubeconfig examples/config/kubeconfig.yaml
.PHONY: k8s-list-namespace

## List all pods
k8s-list-pod:
	kubectl get pods -A --kubeconfig examples/config/kubeconfig.yaml
.PHONY: k8s-list-pod
...
# Template helm chart with default values
helm-template:
	helm template \
		$(HELM_RELEASE_NAME) \
		$(HELM_CHART_DIR) \
...
## Template Dependency-Track Helm chart with recommended defaults
helm-template-dependency-track:
	$(MAKE) helm-template \
...
.PHONY: helm-template-dependency-track
